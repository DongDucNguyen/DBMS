...
def compare():
    ...
    # ── CÁCH 1: DỒN ──────────────────────────────────────────────
    moves, t1_ms = demo_method_1_shift(DELETE_INDEX)

    # ── Khôi phục cho Cách 2 ─────────────────────────────────────
    shutil.copy(backup, ENROLLMENT_FILE)

    moves2, t2_ms = demo_method_2_swap(DELETE_INDEX)
    ...
    moves3, t3_ms = demo_method_3_freelist(DELETE_INDEX)
    ...
    print(f"  {'② Thay Record Cuối (Replace)':<30} {t2_ms:>14.4f}  {moves2:>8} lần  (ghi + truncate)")
    print(f"  {'③ Tombstone + Free List':<30} {t3_ms:>14.4f}  {moves3:>8} lần")
    print(f"  {'-'*57}")
    if t2_ms > 0:
...
if __name__ == '__main__':
    DELETE_INDEX = 4          # Xóa bản ghi thứ 4 (index 0-based)
